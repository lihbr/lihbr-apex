...
float halftone(vec2 uv, float brightness) {
    vec2 cellPos = mod(uv, DOT_SIZE) / DOT_SIZE - 0.5;
    float dist = length(cellPos);
    float radius = (1.0 - brightness) * 0.5;
...
void main() {
	...
	// uv = mat2(cos(0.1), -sin(0.1), sin(0.1), cos(0.1)) * uv;
	vec2 cellCenter = floor(uv / DOT_SIZE) * DOT_SIZE + DOT_SIZE * 0.5;
	vec3 cellImg = texture2D(u_texture, coverScreen(cellCenter, u_resolution, u_texture_resolution.x / u_texture_resolution.y)).rgb;
	float brightness = toGray(cellImg);
	...
	// Composition
	vec4 base = mix(vec4(vec3(0.5), smoothstep(0.2, 0.6, toGray(img))), vec4(FLAMINGO_400, 1.0), icon * 0.9);
	...
	// Halftone
	// base.a = mix(halftone(uv, 0.1) * 0.5 + 0.5, base.a, icon * 0.9);
